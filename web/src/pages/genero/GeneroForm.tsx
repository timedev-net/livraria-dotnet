...
export default function GeneroForm() {
  ...
  const { createGenero, generoShow, updateGeneroById, getGeneroById }: any = useGlobalStore(s => s)
  ...
  async function onSubmit(data: any) {
    if (idParam) {
      updateGeneroById(idParam, {id: idParam, ...data})
    } else {
      createGenero(data)
      console.log(data)
    }
      ...
      <h1 className="text-white mb-5 font-mono w-full">Preencha o formulário corretamente:</h1>
        <div className="flex flex-wrap gap-4 max-w-[60%] justify-center">
          ...
          <a href="#" className="block max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800">
            <h5 className="mb-8 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">Cadastro do Genero</h5>
            <form onSubmit={handleSubmit(onSubmit)}>

              <div className="grid md:grid-cols-2 md:gap-6">
                <div className="relative z-0 w-full mb-6 group">
                  <input {...register("nome")} type="text" className="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer" placeholder=" " required />
